...
void print_status(const msg_t *m, const char *action, int id, gender_t g) {
    printf("%s: студент %d (%s), занято %d/3, внутри %s, стрик: %s %d, осталось: М=%d, Ж=%d\n",
...
void* student_thread(void *arg) {
    ...
    printf("Студент %d (%s) вышел. Время в ванной: %ld сек.\n", id, GENDER_NAME(gender), elapsed);
...
int main(int argc, char *argv[]) {
    ...
    printf("Среднее в ванной: %.2f сек\n", avg_bath);
